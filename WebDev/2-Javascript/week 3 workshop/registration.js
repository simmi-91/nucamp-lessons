...
class Bootcamp {
    ...
    removeStudent(email) {
        const studentExists = this.students.find((s) => s.email === email);
        if (studentExists) {
            var filteredArray = this.students.filter((s) => s.email !== email);
            this.students = filteredArray;
            console.log("Removed student with email:", email);

            return true;
        } else {
            console.log("No student exists with email:", email);
            return false;
        }
    }
    ...
    getInfo() {
        console.log(`This is the Bootcamp for ${this.level} ${this.name} `);
    }
}
...
const runTest = (bootcamp, student) => {
    ...
    bootcamp.registerStudent(new Student("Bugs Bunny", "bugs@bunny.com"));
    bootcamp.registerStudent(new Student("Babs Bunny", "babs@bunny.com"));
    bootcamp.removeStudent("babs22@bunny.com");
